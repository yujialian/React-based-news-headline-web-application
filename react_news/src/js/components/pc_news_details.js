...
export default class PCNewsDetails extends React.Component {
  ...
  componentDidMount() { //Get the new content after the page been loaded, more user friendly.
    ...
    fetch("http://newsapi.gugujiankong.com/Handler.ashx?action=getnewsitem&uniquekey=" + this.props.match.params.uniquekey, myFetchOptions).then(response => response.json()).then(json => {
      ...
      document.title = this.state.newsItem.title + " - React News | React Driven News Platform";
    })
  };
  createMarkup() {
  ...
  render() {
    return (<div>
      ...
      <Row>
        ...
        <Col span={6}>
          <PCNewsImageBlock count={40} type="top" width="400px" cardTitle="Relate News" imageWidth="112px" />
        </Col>
        <Col span={2}></Col>
      </Row>
      <PCFooter></PCFooter>
